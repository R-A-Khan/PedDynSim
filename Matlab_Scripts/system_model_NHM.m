function [ dX ] = system_model_NHM_no_groups(t,X)

global N J m alfa ko kd k1g k2g n_groups d_o d_f include_groups
...
for k=1:length(n_groups)
    for i=sum(n_groups(1:k))-n_groups(k)+1:sum(n_groups(1:k))
        ...
        if include_groups
            ...
            dX(6*i-1)=1/m(i)*(ko*Fe(i,:)*[-sin(X(6*i-3)) cos(X(6*i-3))]'-kd*X(6*i-1));
            dX(6*i)  =1/J(i)*(-kth*a(i)-kom*X(6*i));  
        end
    end
end
