...
The numerical implementation is adapted from the methodology and algorithm developed by Farina et. al. (2017). To simulate a constant flux of pedestrians in the ABCD region, pedestrians are initialised at $t=0$ at equidistant points outside the region, such that the desired flux is maintained as they enter and leave the corridors. Values in $\phi \in [0.2, 0.5, 1, 2]$ are used. The simulations are conducted for two scenarios:
